...
.controller('personSearchController', ['$rootScope', '$scope', '_', '$state',function($rootScope, $scope, _, $state) {
	...
	$scope.refreshPersons = function(personSearch) {
		...
		jQuery.ajax({
			url: YYIMCacheConfig.SERVICE_ADDRESS + YYIMCacheConfig.REST_SERVLET.SEARCH + personSearch,
			type:'get',
			dataType:'json',
			cache:false,
			headers:{
			...
			success:function(result){
				if(!!result && !!result.success && !!result.data){
					for(var x in result.data){
						if(!!result.data[x].imId){
							$scope.searchList.push({
								id: result.data[x].imId,
								name: result.data[x].name,
								email: result.data[x].email,
								office: result.data[x].office,
								chatType: 'chat',
								type: 'chat',
								officePhone: result.data[x].officePhone,
								phoneticName: result.data[x].phoneticName,
								telephone: result.data[x].telephone,
								action:'聊天'
							});
						}
...
.controller('publicAccountController', ['$rootScope', '$scope', "$state", 'publicAccountService', '_', function($rootScope, $scope, $state, publicAccountService, _) {
