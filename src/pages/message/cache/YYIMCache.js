...
YYIMCacheMessageManager.prototype.setUnReadNumToDom = function(){
	var length = YYIMCacheMessageManager.getInstance().unReadedNum;
	length = length > 99 ? "99+" : length;
	if(length){
		jQuery('a[href="#message"] .outer').removeClass('hidden').find('span').html(length);
	}else{
		jQuery('a[href="#message"] .outer').addClass('hidden');
	}
};
